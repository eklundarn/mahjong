...
if (!isLoggedIn()) {
    $login_url = SITE_URL . '/login.php?redirect=' . urlencode('mobile-draft.php');
    $msg = $sv
...
.hand-row-inner { display: grid; padding: 6px 8px; font-size: 0.85em; align-items: center; transition: transform 0.2s; background: white; position: relative; z-index: 1; }
...
.hr-delta { font-weight: 700; font-family: monospace; }
...
.hr-cumul { font-size: 0.78em; color: #333; font-family: monospace; }
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
            <div>
                ...
                <input type="date" id="gameDate" value="<?php echo date('Y-m-d'); ?>"
                       style="width:100%;padding:10px;border:2px solid #ddd;border-radius:8px;font-size:1em;margin-top:4px;">
            </div>
            <div>
...
<div class="mvms-screen" id="screen-game">
    ...
    <div class="mvms-tabs">
        ...
        <a href="?lang=<?php echo currentLang() === 'sv' ? 'en' : 'sv'; ?>" onclick="event.preventDefault();autoSave();location.replace(this.href);" class="mvms-tab" style="text-decoration:none;font-size:1.3em;line-height:1;padding:6px 10px;"><?php echo currentLang() === 'sv' ? '🇬🇧' : '🇸🇪'; ?></a>
    </div>
...
<div class="mvms-screen" id="screen-results">
    ...
    <div style="text-align:center;margin-top:12px;">
        <button class="mvms-btn" onclick="discardGame()" style="background:#999;color:white;font-size:0.85em;padding:6px 16px;"><?php echo $sv ? '🗑 Radera pågående match' : '🗑 Discard game'; ?></button>
    </div>
...
<div class="mvms-dialog" id="pickerDialog">
    <div class="mvms-dialog-box" style="padding:16px;">
        ...
        <input type="search" id="pickerSearch" placeholder="<?php echo $mt['searchPh']; ?>"
               oninput="filterSearch(this.value)" autocomplete="off" inputmode="search"
...
async function processDraftQueue() {
    ...
    while (draftQueue.length > 0) {
        ...
        try {
            const r = await fetch('mobile-draft.php', {
                method: 'POST',
...
async function discardGame() {
    const msg = T.lang === 'sv' 
        ? 'Vill du radera den pågående matchen? All data försvinner och kan inte återställas.'
        : 'Discard the current game? All data will be lost and cannot be recovered.';
...
async function checkServerDrafts() {
    try {
        ...
        if (confirm(msg)) {
            // Load the draft
            const r2 = await fetch('mobile-draft.php', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
